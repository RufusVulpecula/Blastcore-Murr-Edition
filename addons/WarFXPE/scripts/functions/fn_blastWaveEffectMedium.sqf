...
private _lifeTimeVar = random 0.1;
private _sizeVar = random 0.5;

drop [
		["warfxWeps\ParticleEffects\Universal\wave", 1, 0, 1, 0],
		// animationName, type, timerPeriod, lifeTime
		"", "Billboard", 1, 0.4 + _lifeTimeVar,
		// position relative to referenceObject
		[0,0,0],
		// velocity
		[0,0,0],
		// rotation, weight, volume, rubbing, size
		0, 10, 7.9000001, 0.1, [1 + _sizeVar, 1  + _sizeVar, 6 + _sizeVar, 12  + _sizeVar, 24 + _sizeVar, 32 + _sizeVar, 48 + _sizeVar],
		// colors
		[
			[1,1, 0.85, (0.03 + _alphaVar) * _alphaMultiplier],
			[1,1, 0.85, (0.08 + _alphaVar) * _alphaMultiplier],
